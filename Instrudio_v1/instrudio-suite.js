...
(() => {
  ...
  const INSTRUDIO_VERSION = 'v2.3.0';
  const NOTE_INDEX = {C:0,'C#':1,DB:1,D:2,'D#':3,EB:3,E:4,F:5,'F#':6,GB:6,G:7,'G#':8,AB:8,A:9,'A#':10,BB:10,B:11};
  const INSTRUMENTS = {
    guitar:{label:'Guitar', page:'guitar.html'},
    harp:{label:'Harp', page:'harp.html'},
    piano:{label:'Piano', page:'piano.html'},
    violin:{label:'Violin', page:'violin.html'},
    bongo:{label:'Bongos', page:'bongo.html'},
  ...
  function schedulePlayback(payload, bridge, onProgress){ stopPlayback(); const tempo=Math.max(40, Math.min(220, parseFloat(payload.tempo)||96)); const beatMs=60000/tempo; const inst=bridge.instrument || payload.instrument || 'piano'; const songMeta=payload.songId ? SONG_LIBRARY[payload.songId] : null; const settings=songMeta?.arrangements?.[inst] || {}; const transpose=(parseInt(payload.transpose||0,10)||0)+(parseInt(settings.transpose||0,10)||0); const technique=settings.technique || null; if(bridge.init) bridge.init(); if(bridge.setTechnique && technique) bridge.setTechnique(technique);
  ...
  function init(){ installGlobalErrorHandling(); unlockPageAudio(); ensureInfoButton(); addVersionBadge(); const path=location.pathname.split('/').pop()||'index.html'; if(path==='index.html' || path==='') mountHomeConsole(); if(window.InstrudioBridge && path!=='index.html') mountInstrumentPanel(window.InstrudioBridge); }
